...
namespace qic
{
  ...
  namespace
  {
    ...
    inline void set_discord_reg_prob_tol(double a){protect:: _discord_reg_prob_tol = a;}


    template <typename T1>
    typename arma::Col<typename T1::pod_type>::template fixed<3> discord_reg(const T1& rho1,arma::uword nodal, arma::uvec dim)
    {
	...
	throw Exception("qic::discord_reg",Exception::type::INVALID_DIMS);
      ...
      if( dim1 != rho.n_rows)
	throw Exception("qic::discord_reg", Exception::type::DIMS_MISMATCH_MATRIX);
      ...
      for(arma::uword i=0 ; i<3 ; ++i)
	{
	  ...
	  pT S_max = 0.0;
	  if( p1 > static_cast<pT>(protect::_discord_reg_prob_tol) )
	    {
	      rho_1 /= p1;
	      S_max += p1*entropy(rho_1);
	    }
	  if( p2 > static_cast<pT>(protect::_discord_reg_prob_tol)  )
	    {
	      rho_2 /= p2;
