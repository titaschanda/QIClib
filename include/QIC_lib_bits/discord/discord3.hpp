...
namespace qic
{
  ...
  namespace
  {
	...
	pT S_max = 0.0;
	if(p1 > static_cast<pT>(protect::_discord3_prob_tol) )
	  {
	    rho_1 /= p1;
	    S_max += p1 * entropy(rho_1);
	  }
	if(p2 > static_cast<pT>(protect::_discord3_prob_tol) )
	  {
	    rho_2 /= p2;
	    S_max += p2 * entropy(rho_2);
	  }
	if(p3 > static_cast<pT>(protect::_discord3_prob_tol) )
	  {
	    rho_3 /= p3;
	...
	{  
	  ...
	  opt1.set_min_objective(protect::disc_dis3<T1>, (void *)&pass);
      ...
      opt.set_min_objective(protect::disc_dis3<T1>, (void *)&pass);
      ...
      double D = I1 -(S_B- static_cast<pT>(minf) );
