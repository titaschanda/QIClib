...
inline arma::uword gcd(arma::uword m,
                       arma::uword n
                       ) {
...
inline arma::uword gcd(const arma::uvec& M
  ...
  for ( arma::uword i = 1 ; i < M.n_elem ; ++i )
    result = gcd(result, M.at(i));
...
inline arma::uword lcm(arma::uword m,
                       arma::uword n
                       ) {
#ifndef QIC_LIB_NO_DEBUG
  if ( m == 0 || n == 0 )
...
inline arma::ivec real_to_contfrac(double x,
                                   arma::uword n,
                                   arma::uword tol = 1e5
                                   ) {
#ifndef QIC_LIB_NO_DEBUG
...
inline double contfrac_to_real(const arma::ivec& frac,
                               arma::sword n = -1
                               ) {
#ifndef QIC_LIB_NO_DEBUG
  if ( frac.n_elem == 0 )
...
inline arma::uword denominator(double c,
                               arma::uword qmax
                               ) {
#ifndef QIC_LIB_NO_DEBUG
  if ( qmax == 0 )
...
inline arma::sword numerator(double c,
                             arma::uword qmax
                             ) {
#ifndef QIC_LIB_NO_DEBUG
  if ( qmax == 0 )
...
inline arma::uword modexp(arma::uword x,
                          arma::uword a,
                          arma::uword n
                          ) {
#ifndef QIC_LIB_NO_DEBUG
  if ( n == 0 || (a == 0 && n == 0))
    throw Exception("qic::modexp",
...
inline bool is_prime(arma::uword n
  ...
  for ( arma::uword i = 2 ; i <= static_cast<arma::uword>
            (std::llround(std::floor(std::sqrt(n)))) ; i++
        ) {
    if ( n % i == 0 ) {
      return false;
    }
