...
namespace qic
{
		     ...
		     arma::uword dim)
  {
    ...
    arma::Mat< std::complex< pT<T1> > > Eye = 
      arma::eye< arma::Mat< std::complex< pT<T1> > > >(dim,dim);
		   ...
		   arma::uword dim2)
  {
    ...
    std::complex< pT<T1> > I (0.0,1.0);
    arma::Mat< std::complex< pT<T1> > > Eye1 = 
      arma::eye< arma::Mat< std::complex< pT<T1> > > >(dim1,dim1);
    arma::Mat< std::complex< pT<T1> > > Eye2 = 
      arma::eye< arma::Mat< std::complex< pT<T1> > > >(dim2,dim2);
	...
	for(arma::uword j = 0 ; j < dim1 ; ++j)
	  {
		...
		M1.slice(dim1 -1 + count_1) = 
		  ( Eye1.col(i) * Eye1.col(j).t() + 
		    Eye1.col(j) * Eye1.col(i).t() ) 
		  * static_cast< pT<T1> >(std::sqrt(0.5));
	      }
	  }
      }
	...
	for(arma::uword j = 0 ; j < dim2 ; ++j)
	  {
		...
		M2.slice(dim2 -1 + count_3) = 
		  (Eye2.col(i) * Eye2.col(j).t() 
		   + Eye2.col(j) * Eye2.col(i).t() ) 
		  * static_cast< pT<T1> >(std::sqrt(0.5));
	      }
	  }
      }
