...
namespace qic
{
  ...
  TR expm_sym(const T1& rho1)
  {
    ...
    return 
      eigvec 
      * arma::diagmat(arma::exp(arma::conv_to< arma::Col<
				eT<T1> 
				> >::from(eigval))) 
      * eigvec.t();
  }
  ...
  TR expm_gen(const T1& rho1)
  {
    ...
    const double log2_val = (norm_val > static_cast< pT<T1> >(0)) ? 
      static_cast<double>(std::log2(norm_val)) : static_cast<double>(0);
    
    int exponent = int(0);  
    std::frexp(log2_val, &exponent);
    const arma::uword s = 
      static_cast<arma::uword>( std::max(static_cast<int>(0), 
					 exponent + static_cast<int>(1)) );
    ...
    arma::Mat< eT<T1> > E(A.n_rows,A.n_cols,arma::fill::eye); E  +=  c*A;
    arma::Mat< eT<T1> > D(A.n_rows,A.n_cols,arma::fill::eye); D  -= c*A;
