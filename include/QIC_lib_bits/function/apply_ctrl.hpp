...
namespace qic 
{
		  ...
		  arma::uvec dim)
  {
    ...
    if(ctrlsys.n_elem > dim.n_elem 
       || arma::find_unique(ctrlsys).eval().n_elem != ctrlsys.n_elem 
    ...
    for(arma::sword i=n-2;i>=0; --i)
      product(i)=product(i+1)*dim(i+1);
    
    arma::uvec productr=arma::ones<arma::uvec>(m);
    for(arma::sword i = m-2 ; i>-1 ; --i)
      productr.at(i)=productr.at(i+1)*dim.at(sys(i));
    ...
    if(!checkV)
      {
	arma::Col<eTR> rho = arma::zeros< arma::Col<eTR> >(p.n_rows);   
	...
	while(loop_counter[loop_no]==0)
	  {
	    ...
	    for(arma::uword i = 0 ; i < n ; ++i)
	      {
		count1 += (arma::any(ctrl == i+1) && loop_counter[i] != 0 ) ? 
		  1 : 0;
		...
		arma::uword I(0),J(0),K(0), L(0), counter(0);
		...
		for(arma::uword i = 0 ; i < n ; ++i)
		  {
			...
			for (arma::uword j = 1 ; j < o ; ++j)
			  counter_1 += loop_counter[ctrl.at(0)-1] 
			    == loop_counter[ctrl.at(j)-1] ? 
			    1 : 0;

			power = counter_1 == o ? 
			  loop_counter[ctrl.at(0)-1] : 0 ;
		      }
	...
	return rho;
      ...
      {
	arma::Mat< eT<T2> > U =
	  arma::zeros< arma::Mat< eT<T2> > >(p.n_rows,p.n_rows);   
