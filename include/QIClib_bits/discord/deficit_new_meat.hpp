...
double def_def2(const std::vector<double>& x, std::vector<double>& grad,
               void* my_func_data) {
  (void)grad;
...
template <typename T1>
double def_def32(const std::vector<double>& x, std::vector<double>& grad,
                void* my_func_data) {
  (void)grad;
  std::complex<trait::pT<T1> > I(0.0, 1.0);
...
inline deficit_space<T1>::deficit_space(const T1& rho1, arma::uword nodal,
  ...
  arma::uword n =
      static_cast<arma::uword>(std::llround(std::log(_rho.n_rows) / std::log(dim)));
...
template <typename T1>
inline deficit_space<T1>&
deficit_space<T1>::angle_range(const arma::vec& a) {
#ifndef QICLIB_NO_DEBUG
...
template <typename T1>
inline deficit_space<T1>&
deficit_space<T1>::initial_angle(const arma::vec& a) {
#ifndef QICLIB_NO_DEBUG
  if (_deficit2 && a.n_elem != 2)
    throw Exception(
...
template <typename T1> inline deficit_space<T1>& deficit_space<T1>::compute() {
  ...
  if (_deficit2) {
    ...
    if (_deficit_global == true) {
      ...
      opt1.set_min_objective(protect::def_def2<T1>, static_cast<void*>(&pass));
    ...
    opt.set_min_objective(protect::def_def2<T1>, static_cast<void*>(&pass));
  ...
  if (_deficit3) {
    ...
    protect::TO_PASS_def3<arma::Mat<trait::eT<T1> > > pass(_rho, eye2, eye3, eye4,
                                                           _nodal, _party_no);
    ...
    if (_deficit_global == true) {
      ...
      opt1.set_lower_bounds(lb);
      opt1.set_upper_bounds(ub);
      opt1.set_min_objective(protect::def_def32<T1>, static_cast<void*>(&pass));
      opt1.set_xtol_rel(_deficit_global_xtol);
      opt1.set_ftol_rel(_deficit_global_ftol);
      opt1.optimize(x, minf1);
    }
    ...
    opt.set_lower_bounds(lb);
    opt.set_upper_bounds(ub);
    opt.set_min_objective(protect::def_def32<T1>, static_cast<void*>(&pass));
    opt.set_xtol_rel(_deficit_local_xtol);
    opt.set_ftol_rel(_deficit_local_ftol);
    opt.optimize(x, minf);
